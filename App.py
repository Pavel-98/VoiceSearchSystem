...
def search_thread():
    ...
    input_request()
    #delete('request.wav')
    request = recognize_speech('request.wav', defaultLanguage.get())#u
    '''єєif not request:
        stop()
        return
        pass'''
    ...
    #delete('request.wav')
    SearchProcess.initialize_process(text_search.get(), voice_search.get())
    SearchProcess.StartProcess(request, defaultLanguage.get())#start(request, defaultLanguage.get())
    #№delete('request.wav')
